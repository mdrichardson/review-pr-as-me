...
- PR URL mode (create immediately): `"Parse PR URL and fetch metadata"` → `"Fetch existing PR comments"` → `"Prepare detached review worktree"` → `"Detect codebase standards"`
...
from the detached review worktree. Set `repo_path` to `worktree_path`
and pass the full path to each phase as
`repo_path`.
