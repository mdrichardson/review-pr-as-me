---
name: rprf
description: "Fast variant of rpr — same workflow but with every model stepped down one tier and Agent 5 (scope match) skipped. Use on subsequent runs after an initial /rpr pass, when you're re-verifying fixes rather than reviewing a PR fresh. Triggers on /rprf, 'fast review', 'quick re-review', 're-review after fixes'."
...
This skill inherits the `rpr` skill's host mappings. The relative
`../rpr/SKILL.md` path is the standalone plugin layout used by both
Claude Code and Copilot CLI. The absolute fallbacks below also support
each host's personal-skill layout.

## Step 0 — Load the rpr workflow first

Read `../rpr/SKILL.md` (relative to this file's directory) in full
...
back to `~/.claude/plugins/mdrichardson/review-pr-as-me/skills/rpr/SKILL.md`
under Claude Code or
`~/.copilot/skills/rpr/SKILL.md` under Copilot CLI.

If neither resolves, halt and report the error — fast mode only
documents the deltas from `rpr`, so it can't run a review without
loading `rpr` first.

**Important:** `rpr` is the canonical full workflow. Reading it gives
you the phase index, invocation contract, hard rules, and cross-cutting
invariants, while its phase implementations live in
`../rpr/phases/*.md` and are loaded JIT as you enter each phase.
...
| Delta | Applies in |
|-------|------------|
| Model stepdown table (below) | `../rpr/phases/03-analyze.md` (analysis dispatch) and `../rpr/phases/04-verify.md` (per-finding verification) |
| Skip Agent 5 (Scope Match) rule | `../rpr/phases/03-analyze.md` (dispatch) and the conditional-task creation at the end of `../rpr/phases/02-pre-compute.md` |
| Task-list adjustments (verification task wording, omit Scope-match) | `../rpr/phases/01-setup.md` (Required Tasks block) and end of `../rpr/phases/02-pre-compute.md` (conditional tasks) |
| Cache-hit shortcut announcement | `../rpr/phases/02-pre-compute.md` (PR-level slot check at the top of the phase) |
...
when the `suppression_list` is non-empty (same condition as `rpr`).

Agent 8 also runs at haiku in `rpr` — no lower tier to step down to.
Fast mode dispatches Agent 8 unconditionally, same as `rpr`.
...
The rest of the `rpr` skill — its Phase 1 (setup, clone, local-vs-PR
...
is the dominant fast-mode optimization. When `rprf` runs after an
`rpr` review with no new commits since, Phase 2 (pre-compute)
...
to step down. Fast mode does the same work as `rpr` here.

Task-list adjustments (apply at the points the `rpr` flow creates tasks):
- Omit any "Scope match" analysis task (Phase 2 end-of-phase conditional task creation).
- Keep the "Suppression removability" analysis task (no change from `rpr` — still conditional on a non-empty `suppression_list`).
- Keep the "Fetch existing PR comments" setup task and the "Classify previous comments" wrap-up task unchanged from `rpr` (both conditional on PR URL mode + non-empty comment list).
- The wrap-up verification task becomes `"Verify findings ({N} parallel Sonnet)"`.

## When to use rprf vs rpr

- **`/rpr`** — first-pass review, unfamiliar PR, high-stakes
  merge, or whenever scope-match against the PR description matters.
- **`/rprf`** — re-review after fixes have landed, iterating
